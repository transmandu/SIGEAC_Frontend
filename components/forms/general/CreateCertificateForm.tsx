...
import {
  ...
  useGetEmployeesList,
} from "@/hooks/sms/useGetCertificates";
...
export const CreateCertificateForm = ({
  ...
  const { data: courses, isLoading: loadingCourses } =
    useGetSMSCoursesList(companySlug);
  const { data: employees, isLoading: loadingEmployees } =
    useGetEmployeesList(companySlug);
  const { mutateAsync: createCertificate, isPending } =
    useCreateSMSCertificate();

  // --- CAMBIO: Lógica de carga unificada ---
  const isDataLoading =
    loadingCourses ||
    (user?.roles?.some((r) =>
      ["JEFE_SMS", "ANALISTA_SMS", "SUPERUSER"].includes(r.name.toUpperCase()),
    ) &&
      loadingEmployees);
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 py-2">
      {isManagement ? (
        <div
          className="grid gap-2 p-3 border rounded-lg bg-accent/20"
          data-tour="cert-create-empleado"
        >
          <Label
            htmlFor="employee_dni"
          ...
          <input
            type="hidden"
            {...register("employee_dni", { required: isManagement })}
          />
          {errors.employee_dni && (
      ...
      <div className="grid gap-2" data-tour="cert-create-curso">
      ...
      <div className="grid gap-2" data-tour="cert-create-archivo">
        ...
        >
          <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center px-2">
            {fileName ? (
              ...
              <>
                <UploadCloud
                  className={`w-8 h-8 mb-2 transition-colors ${isDragging ? "text-blue-500" : "text-muted-foreground"}`}
                />
