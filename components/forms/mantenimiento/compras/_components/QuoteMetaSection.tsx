...
export function QuoteMetaSection({
  ...
  const isGeneralQuote = useWatch({ control: form.control, name: "is_general_quote" });
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-stretch">
      ...
      <div className="h-full rounded-xl border bg-muted/15 overflow-hidden">
        ...
        <div className="flex flex-col gap-3 p-3">
          ...
          {/* Cotización de artículos generales/ferretería */}
          <FormField
            control={form.control}
            name="is_general_quote"
            render={({ field }) => (
                ...
                <FormControl>
                  <Checkbox
                    ...
                    onCheckedChange={(checked) => {
                      ...
                      if (checked) form.setValue("vendor_id", "");
                    }}
                  />
                </FormControl>
                <FormLabel className="m-0 text-[11px] font-medium text-muted-foreground cursor-pointer">
                  Cotización de artículos generales/ferretería (sin proveedor)
                </FormLabel>
              </FormItem>
            )}
          />
            ...
            render={({ field }) => (
              <FormItem className="space-y-2">
                <div className="flex items-center gap-2 min-h-[16px] pb-1.5">
                  <FormLabel className="m-0 text-[11px] font-medium uppercase tracking-wide text-muted-foreground">
                    Proveedor
                    <RequiredIndicator show={!isGeneralQuote} />
                  </FormLabel>
                  <div className="h-px flex-1 bg-border/60" />
                </div>
                ...
                <Popover open={openVendor} onOpenChange={setOpenVendor}>
                  <PopoverTrigger asChild>
                    <FormControl>
                      <Button
                        variant="outline"
                        disabled={isVendorsLoading || isGeneralQuote}
