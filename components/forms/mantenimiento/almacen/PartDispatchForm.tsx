...
import { AlertCircle, Building2, CalendarIcon, Check, ChevronsUpDown, Loader2, Plane } from "lucide-react";
...
export function PartDispatchForm({ onClose }: FormProps) {
  const { user } = useAuth();
  ...
  const { data: batches, isPending: isBatchesLoading, isError: isBatchesError } = useGetBatchesWithInWarehouseArticles({
  ...
  const getMaxQuantity = (): number => articleSelected?.quantity || 0;
  ...
  const validateAndAdjustQuantity = (value: string): string => {
    ...
    if (maxQuantity > 0 && numericValue > maxQuantity) {
      setQuantityError(`Se ajustó a la cantidad máxima disponible: ${maxQuantity} ${articleSelected.unit}`);
  ...
  useEffect(() => setValue("department_id", ""), [isDepartment, setValue]);
  useEffect(() => { setQuantity("1"); setQuantityError(""); setShowAutoAdjustMessage(false); }, [articleSelected]);
  useEffect(() => {
    if (!articleSelected) return;
    if (!form.getValues("articles")?.length) return;
    setValue("articles.0.quantity", parseFloat(quantity) || 0);
  }, [quantity, articleSelected, form, setValue]);

  const handleArticleSelect = (id: number, serial: string | null, batch_id: number) => {
    const selected = batches?.flatMap((b: BatchesWithCountProp) => b.articles).find(a => a.id === id);
    if (!selected) return;

    setValue("articles", [{ article_id: id, serial: serial ?? null, quantity: 1, batch_id }]);
  ...
  const handleQuantityChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    setQuantity(validateAndAdjustQuantity(e.target.value));
  };
  ...
  const onSubmit = async (data: FormSchemaType) => {
    if (articleSelected) {
      ...
      if (maxQuantity > 0 && q > maxQuantity) {
        setQuantityError(`No puede retirar más de ${maxQuantity} ${articleSelected.unit} disponibles`);
        return;
      }
    }
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col space-y-6 w-full">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide px-2">Personal Responsable</span>
            <div className="h-px flex-1 bg-border/60" />
          </div>
            ...
            <FormField control={form.control} name="requested_by" render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={field.onChange}>
                  ...
                  <SelectContent>
                    {employeesLoading && <div className="flex items-center justify-center py-4"><Loader2 className="size-4 animate-spin text-muted-foreground" /></div>}
                    {!employeesLoading && employeesError && <div className="py-4 text-center text-sm text-muted-foreground">Error al cargar empleados</div>}
                    {employees?.map((emp) => (
                      <SelectItem key={emp.id} value={`${emp.dni}`}>{emp.first_name} {emp.last_name} - {emp.job_title.name}</SelectItem>
                    ))}
                  </SelectContent>
                </Select>
                <FormMessage />
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-1">
            <div className="h-px flex-1 bg-border/60" />
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide px-2">Información de la Solicitud</span>
            <div className="h-px flex-1 bg-border/60" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {/* Work Order */}
            <FormField control={form.control} name="work_order" render={({ field }) => (
              <FormItem>
                ...
                <FormControl><Input className="w-full resize-none" placeholder="Ej: Se necesita para la limpieza..." {...field} /></FormControl>
                <FormMessage />
              </FormItem>
            )} />

            {/* Fecha */}
            <FormField control={form.control} name="submission_date" render={({ field }) => (
            ...
            <FormItem>
              <div className="flex items-center justify-between mb-2 gap-2">
                <FormLabel className="text-sm font-medium flex items-center gap-2">
                  {isDepartment ? <Building2 className="h-4 w-4 text-muted-foreground" /> : <Plane className="h-4 w-4 text-muted-foreground" />} Destino
                </FormLabel>
                <div className="flex items-center gap-1">
                  <Checkbox id="is-department" checked={isDepartment} onCheckedChange={(c) => setIsDepartment(c as boolean)} className="h-4 w-4" />
                  <label htmlFor="is-department" className="text-xs font-medium leading-none cursor-pointer text-muted-foreground">¿Es para un departamento?</label>
                </div>
              </div>
              <Select onValueChange={field.onChange} defaultValue={field.value} disabled={isDepartment ? isDepartmentsLoading : isAircraftsLoading}>
                <FormControl><SelectTrigger className="h-10"><SelectValue placeholder={isDepartment ? "Seleccione un departamento..." : "Seleccione una aeronave..."} /></SelectTrigger></FormControl>
                <SelectContent>
                  {isDepartment
                    ? departments?.map(dep => <SelectItem key={dep.id} value={dep.id.toString()}>{dep.name}</SelectItem>)
                    : aircrafts?.map(ac => <SelectItem key={ac.id} value={ac.id.toString()}>{ac.acronym} - {ac.manufacturer?.name ?? "Sin fabricante"}</SelectItem>)
                  }
                </SelectContent>
              </Select>
              <FormMessage />
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 mb-1">
            <div className="h-px flex-1 bg-border/60" />
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide px-2">Parte a Retirar</span>
            <div className="h-px flex-1 bg-border/60" />
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 items-end">
            <div className="lg:col-span-2">
              <FormField control={form.control} name="articles" render={() => (
                <FormItem className="flex flex-col">
                  <FormLabel className="text-sm font-medium">Parte a Retirar</FormLabel>
                  <Popover open={open} onOpenChange={setOpen}>
                    <PopoverTrigger asChild>
                      <Button variant="outline" role="combobox" aria-expanded={open} className="w-full justify-between h-10" disabled={isBatchesLoading}>
                        {isBatchesLoading ? <><Loader2 className="h-4 w-4 animate-spin mr-2" />Cargando...</> : articleSelected ? <span className="truncate">{articleSelected.serial ?? articleSelected.part_number}</span> : <span className="text-muted-foreground">Seleccione la parte...</span>}
                    ...
                    <PopoverContent className="w-full p-0" align="start">
                      <Command>
                        ...
                        <CommandList>
                          ...
                          <>
                            <CommandEmpty>No se han encontrado partes...</CommandEmpty>
                            {batches?.map(batch => (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                {articleSelected && <Button type="button" variant="ghost" size="sm" onClick={setToMaxQuantity} className="h-7 text-xs text-primary hover:text-primary">Usar máximo disponible</Button>}
              </div>
              <Input type="text" min="0" step="1" max={articleSelected?.quantity || undefined}
