...
import {
    Check, ChevronsUpDown, Loader2, PackagePlus, Plus, X,
} from "lucide-react"
...
import { useWorkshopDispatchForm, aeroKey, genKey, type ItemCategory } from "./_hooks/useWorkshopDispatchForm"
...
export function WorkshopDispatchForm({ onClose }: FormProps) {
    const {
        form, user, onSubmit, createWorkshopDispatch,
        openAdd, setOpenAdd, addCategory, setAddCategory,
            ...
            <form onSubmit={form.handleSubmit(onSubmit)} className="flex w-full min-w-0 flex-col space-y-6">
                ...
                <div className="space-y-4">
                    ...
                    <div className="grid grid-cols-1 items-start gap-4 md:grid-cols-2 *:min-w-0">
                        <div className="space-y-2">
                            ...
                            <p className="text-xs text-muted-foreground">Usuario actual que registra la salida.</p>
                        </div>
                                ...
                                <FormItem>
                                    <div className="flex min-h-6 items-center justify-between gap-2">
                                        ...
                                        <CreateWorkshopDialog
                                            onSuccess={(workshop) => field.onChange(workshop.id.toString())}
                                            triggerButton={
                                                <Button
                                                    type="button"
                                                    variant="ghost"
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-3 *:min-w-0">
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl>
                                        <Input className="h-10 w-full" placeholder="Quién solicita la salida" {...field} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                        ...
                        <FormField
                            ...
                            render={({ field }) => (
                                <FormItem>
                                    <FormLabel className="text-sm font-medium">Recibe en el taller</FormLabel>
                                    <FormControl>
                                        <Input className="h-10 w-full" placeholder="Persona que recibe en el taller" {...field} />
                                    </FormControl>
                                    <FormMessage />
                                </FormItem>
                            )}
                ...
                <div className="space-y-4">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium flex items-center justify-between">
                            ...
                            <span className="text-xs text-muted-foreground">
                                Aeronáuticos: {aeronauticalCount} · General/Ferretería: {generalCount}
                            </span>
                        </label>

                        <Popover open={openAdd} onOpenChange={(v) => { setOpenAdd(v); if (v) setAddCategory("component") }}>
                            ...
                            <PopoverContent className="p-0" align="start" matchTriggerWidth>
                                <div className="relative border-b p-2">
                                    <div className="flex flex-wrap items-center justify-center gap-2 px-10">
                                        ...
                                        <Button
                                            type="button"
                                            variant={(addCategory as string) === "general" ? "default" : "outline"}
                                            className="h-8"
                    ...
                    {aeroFA.fields.length > 0 && (
                        <div className="space-y-2">
                            <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Aeronáuticos</p>
                            {aeroFA.fields.map((f, index) => {
                                ...
                                return (
                                    <ConsumableArticleRow
                                        ...
                                        showConversionPanel={convState.target === "aero" && convState.rowFieldId === f.id && !!article && isAeroConsumable(f.id)}
                ...
                <div className="space-y-4">
                    ...
                    <FormField
                        ...
                        render={({ field }) => (
                            <FormItem>
                                <FormControl>
                                    <Textarea rows={4} className="w-full resize-none" placeholder="Ej: Se envía a overhaul por..." {...field} />
                                </FormControl>
                                <FormMessage />
                            </FormItem>
                        )}
                ...
                <div className="flex justify-end gap-3 pt-2">
                    ...
                    <Button
                        ...
                        disabled={createWorkshopDispatch?.isPending || aeronauticalCount + generalCount === 0 || hasBlockingQtyError || hasInvalidQty}
                        type="submit"
                    >
