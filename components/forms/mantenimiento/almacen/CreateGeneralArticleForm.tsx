...
import {
  useAddQuantityGeneralArticle,
  useCreateGeneralArticle,
  useUpdateGeneralArticle,
} from "@/actions/mantenimiento/almacen/inventario/articulos_generales/actions";
...
const CreateGeneralArticleForm = ({
  ...
  const form = useForm<FormValues>({
    ...
    values: {
      // Usar 'values' en lugar de 'defaultValues' para que reaccione a props
      mode: isEditing ? "edit" : useExisting ? "add" : "create",
      description: isEditing
        ? (initialData?.description ?? "")
        : (selectedArticle?.description ?? ""),
      brand_model: isEditing
        ? (initialData?.brand_model ?? "")
        : (selectedArticle?.brand_model ?? ""),
      variant_type: isEditing
        ? (initialData?.variant_type ?? "")
        : (selectedArticle?.variant_type ?? ""),
      ...
      minimum_quantity: isEditing
        ? (initialData?.minimum_quantity ?? undefined)
        : undefined,
      maximum_quantity: isEditing
        ? (initialData?.maximum_quantity ?? undefined)
        : undefined,
      warehouse_id:
        (initialData?.warehouse_id ?? initialData?.warehouse?.id)?.toString() ??
        "2",
    },
  });
  ...
  const busy =
    ...
    updateGeneralArticle?.isPending;
      ...
      if (values.mode === "add") {
        ...
        await updateGeneralArticle.mutateAsync({
          ...
          articleData: {
            ...
            minimum_quantity:
              values.minimum_quantity !== undefined
                ? parseFloat(values.minimum_quantity.toFixed(2))
                : undefined,
            maximum_quantity:
              values.maximum_quantity !== undefined
          ...
          dimension:
            dimensionPayload(dimension, !!initialData?.dimension) ?? undefined,
        });
      } else {
  ...
  return (
    <Form {...form}>
      ...
      >
        <div
          className={cn("shrink-0 space-y-1", inDialog && "border-b px-6 py-4")}
        >
          <h2 className="text-xl font-semibold tracking-tight">
            {isEditing
              ? "Editar Artículo General"
              : "Registrar Artículo General"}
          </h2>
          ...
          {!isEditing && (
            <div className="rounded-lg border bg-muted/40 p-4 space-y-3">
              <label className="flex items-start gap-3 cursor-pointer">
                ...
                <span className="space-y-0.5">
                  ...
                  <span className="block text-xs text-muted-foreground">
                    En vez de crear uno nuevo, incrementa la cantidad de uno ya
                    registrado.
                  </span>
                </span>
              </label>
              ...
              {useExisting && (
                <Select
                  onValueChange={(val) => {
                    const art = generalArticles?.find(
                      (a) => a.id.toString() === val,
                    );
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormControl>
                          <Input
                            {...field}
                            placeholder="Ej: Caja de 50, 3/4 pulgada"
                            className={fieldClass}
                            disabled={currentMode === "add"}
                          />
                        </FormControl>
              ...
              <div className="grid grid-cols-[1fr_1.6fr] gap-3 items-start">
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormLabel className={labelClass}>
                        {currentMode === "add" ? "Sumar cantidad" : "Cantidad"}
                      </FormLabel>
                      <FormControl>
                        <NumericInput
                          {...field}
                        ...
                        >
                          <FormControl>
                            <SelectTrigger className={selectTriggerClass}>
                              <SelectValue
                                placeholder={
                                  isUnitsLoading ? "Cargando..." : "Seleccione"
                                }
                              />
                            </SelectTrigger>
                          </FormControl>
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <FormDescription className="text-xs">
                          Opcional. Nivel de stock a reponer, no un tope de
                          existencia.
                        </FormDescription>
                        <FormMessage />
                      </FormItem>
