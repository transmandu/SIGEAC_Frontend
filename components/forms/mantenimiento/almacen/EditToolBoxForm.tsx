...
import {
  useCreateToolBox,
  useUpdateToolBox,
} from "@/actions/mantenimiento/almacen/inventario/caja_herramientas/actions";
...
export function EditToolBoxForm({ onClose, initialData }: FormProps) {
  ...
  const { data: employees, isLoading: employeesLoading } =
    useGetEmployeesForBox(selectedStation ?? null, selectedCompany!.slug);

  const { data, isLoading } = useGetEditToolBoxTools(
  ...
  const form = useForm<FormSchemaType>({
    ...
    defaultValues: {
      ...
      tool_id:
        initialData.tool.map((tool) => tool.article.id!.toString()) || [],
    },
  });
  ...
  useEffect(() => {
    if (initialData.tool) {
      const preselectedTools = initialData.tool.map((tool) =>
        tool.article.id!.toString(),
      );
  ...
  return (
    <Form {...form}>
      ...
      >
                            ...
                            {batch.article.map((article) => (
                              ...
                              >
                                <Check
                                  className={cn(
                                    "mr-2 h-4 w-4",
                                    isToolSelected(article.id!.toString())
                                      ? "opacity-100"
                                      : "opacity-0",
                                  )}
                                />
                                <p className="text-sm text-muted-foreground italic">
                                  SN - {article.tool.serial}
                                </p>
        ...
        <Button disabled={updateToolBox.isPending}>
          {updateToolBox.isPending ? (
