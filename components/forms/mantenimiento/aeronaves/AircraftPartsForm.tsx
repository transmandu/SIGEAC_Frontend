...
const fmtNumber = (n: unknown): string => {
  ...
  const isEuropean =
    lastComma > lastDot || (lastComma !== -1 && lastDot === -1);
...
const PartSchema: any = z.object({
  ...
  cycles_since_new: z
    ...
    .min(0)
    .optional(), // Cycles Since New (entero)
  cycles_since_overhaul: z
    .number()
    .int("Debe ser un número entero")
    .min(0)
    .optional(), // Cycles Since Overhaul (entero)
  condition_type: z.enum(["NEW", "OVERHAULED"]),
  is_father: z.boolean().default(false),
  sub_parts: z.array(z.lazy(() => PartSchema)).optional(),
  batch_id: z.string().min(1, "La descripción es obligatoria"),
  condition_id: z.string().min(1, "La condición el obligatoria"),
});
...
export function AircraftPartsInfoForm({
  onNext,
  onBack,
  initialData,
}: {
  ...
  const [notApplicableCategories, setNotApplicableCategories] = useState<
    Record<string, boolean>
  >({
    ENGINE: false,
    APU: false,
  ...
  const partsByCategory = fields.reduce(
    (acc, field, index) => {
    ...
    {} as Record<
      string,
      Array<{ field: FieldArrayWithId<PartsFormType, "parts">; index: number }>
    >,
  );
  ...
  const removeItem = (fullPath: string) => {
    ...
    if (fields.length <= 1) {
      toast({
        ...
        description:
          "Debe haber al menos una parte registrada en alguna categoría",
        variant: "destructive",
      });
      return;
    }
  ...
  // Obtener los datos de useGetBatchesByCategory y useGetConditions
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 mt-8">
        <ScrollArea className="h-[600px]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6 px-1">
            {Object.entries(PART_CATEGORIES).map(
              ([categoryKey, categoryLabel]) => {
                ...
                return (
                  ...
                  >
                    ...
                    `}
                      onClick={() =>
                        !isNotApplicable && toggleCategory(categoryKey)
                      }
                    >
                      ...
                      `}
                        >
                          <Icon
                            className={`size-5 ${isNotApplicable ? "text-gray-400" : colorClasses.color}`}
                          />
                        </div>
                          ...
                          {!isNotApplicable && (
                            <div className="transition-transform duration-200">
                              {expandedCategories[categoryKey] ? (
                                <ChevronDown
                                  className={`size-4 ${colorClasses.color}`}
                                />
                              ) : (
                                <ChevronRight
                                  className={`size-4 ${colorClasses.color}`}
                    ...
                    {!isNotApplicable && (
                      <div
                        className={`h-px ${colorClasses.bgColor} opacity-60`}
                      />
                    )}
                    ...
                    `}
                      >
                        <Checkbox
                          ...
                          onCheckedChange={() =>
                            toggleNotApplicable(categoryKey)
                          }
                        ...
                        <>
                          ...
                          {!hasContent && (
                            <div className="text-center py-6 mb-3">
                              <div
                                className={`inline-flex p-3 rounded-full ${colorClasses.bgColor} mb-2`}
                              >
                                <Icon
                                  className={`size-6 ${colorClasses.color} opacity-40`}
                                />
                              </div>
                              <p className="text-sm text-muted-foreground">
                                No hay {categoryLabel.toLowerCase()}s
                                registrados
                              </p>
                            </div>
                          )}
                          ...
                          <Button
                            ...
                            onClick={(e) => {
                              e.stopPropagation();
                              addPartWithCategory(
                                categoryKey as keyof typeof PART_CATEGORIES,
                              );
                            }}
                            className={`
                          ...
                          >
                            <PlusCircle
                              className={`size-4 mr-2 ${colorClasses.color}`}
                            />
          ...
          {partsByCategory["uncategorized"] &&
            partsByCategory["uncategorized"].length > 0 && (
              <div className="border-2 border-dashed border-yellow-400 dark:border-yellow-600 rounded-lg p-4 bg-yellow-50 dark:bg-yellow-950/20">
                ...
                <div className="space-y-2">
                  {partsByCategory["uncategorized"].map(
                    ({
                      field,
                      index,
                    }: {
                      field: FieldArrayWithId<PartsFormType, "parts">;
                      index: number;
                    }) => (
                      <PartSection
                        key={field.id}
...
function PartSection({
  ...
  isConditionsLoading?: any[];

}) {
  const hassub_parts = form.watch(
    `${path}.is_father` as `parts.${number}.is_father`,
  );
  const sub_parts =
    form.watch(`${path}.sub_parts` as `parts.${number}.sub_parts`) || [];
  const category = form.watch(`${path}.category` as `parts.${number}.category`);
  
  const batchNameById = new Map();
  batches?.forEach((b: any) => batchNameById.set(String(b.id), b.name));
          ...
          <h3 className="font-medium">
            ...
            {category && (
              <span className="text-muted-foreground ml-2">
                ({PART_CATEGORIES[category as keyof typeof PART_CATEGORIES]})
              </span>
            )}
          </h3>
        </div>
        ...
        <div className="space-y-4 pl-6">
            ...
            <FormField
              ...
              render={({ field }) => (
                <ManufacturerCombobox
                  value={field.value}
                  onChange={(value) =>
                    form.setValue(`${path}.manufacturer_id` as any, value)
                  }
                  manufacturers={manufacturers}
                  label="Marca"
          ...
          {hassub_parts && (
            <div className="mt-4 pl-4 border-l-2 border-gray-200">
              ...
              {hassub_parts &&
                sub_parts.map((_: unknown, subIndex: number) => (
                  <div
                    key={subIndex}
