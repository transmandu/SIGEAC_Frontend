...
import { useDeleteSecondaryUnit } from "@/actions/ajustes/globales/unidades/actions"
import { Loader2, MoreHorizontal, Trash2 } from "lucide-react"
import { useState } from "react"
import { Button } from "../../ui/button"
import { Dialog, DialogContent, DialogDescription, DialogFooter, DialogHeader, DialogTitle, DialogTrigger } from "../../ui/dialog"

const SecondaryUnitDropdownActions = ({ id }: { id: string | number }) => {
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter className="flex flex-col gap-2 md:gap-0">
          ...
          <Button disabled={deleteSecondaryUnit.isPending} className="hover:bg-white hover:text-black hover:border hover:border-black transition-all" onClick={() => handleDelete(id)}>{deleteSecondaryUnit.isPending ? <Loader2 className="size-4 animate-spin" /> : <p>Confirmar</p>}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
