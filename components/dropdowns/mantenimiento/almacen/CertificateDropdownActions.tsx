...
const CertificateDropdownActions = ({ id }: { id: string | number }) => {
  ...
  const handleDelete = async (id: number | string) => {
    await deleteCertificate.mutateAsync({id, company: selectedCompany!.slug});
    setOpen(false);
  }
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        ...
        <DialogFooter className="flex flex-col gap-2 md:gap-0">
          ...
          <Button disabled={deleteCertificate.isPending} className="hover:bg-white hover:text-black hover:border hover:border-black transition-all" onClick={() => handleDelete(id)}>{deleteCertificate.isPending ? <Loader2 className="size-4 animate-spin" /> : <p>Confirmar</p>}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
...
export default CertificateDropdownActions
