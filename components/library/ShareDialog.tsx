...
import {
  ...
  Download, List, Plus, History, Eye, ArrowLeft, Send, CheckCircle2, Share2
} from "lucide-react";
...
export default function ShareDialog({ isOpen, onClose, doc, company, isDipDirector }: ShareDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (showCustomDuration && (!customHours || durationValue < 24 || durationValue > 168)) {
      ...
      <DialogContent className="sm:max-w-md bg-white dark:bg-[#141618] border-slate-300 dark:border-gray-800 shadow-2xl p-0 overflow-hidden outline-none rounded-[2rem]">
        <div className="bg-white dark:bg-gray-800/30 border-b border-slate-200 dark:border-gray-800">
          ...
          <div className="flex px-6 gap-8">
            <button
              onClick={() => { setActiveTab('form'); setGeneratedUrl(''); setConfirmed(false); setSelectedShareUrl(null); }}
            ...
            >
              <div className="flex items-center gap-2">
                {isDipDirector ? <Plus className="h-3.5 w-3.5" /> : <Send className="h-3.5 w-3.5" />}
                {isDipDirector ? 'Generar Acceso' : 'Solicitar Acceso'}
              </div>
              {activeTab === 'form' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600" />}
            </button>
            {hasActiveShares && (
              <button
                onClick={() => { setActiveTab('active'); setSelectedShareUrl(null); }}
              ...
              >
                ...
                {activeTab === 'active' && <div className="absolute bottom-0 left-0 w-full h-0.5 bg-blue-600" />}
              </button>
            )}
          </div>
        </div>
          ...
          {activeTab === 'active' ? (
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="flex items-center gap-4">
                            <span className="flex items-center gap-1.5 text-[9px] font-bold bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 px-2.5 py-1 rounded-full uppercase border border-blue-100 dark:border-blue-800/50">
                              ...
                              {share.version?.version_label || share.version?.version_number || 'ACTUAL'}
                            </span>
            ...
            <div className="flex flex-col items-center animate-in zoom-in-95 duration-300 py-2">
              ...
              <div className="flex gap-3 w-full">
                ...
                <Button
                  onClick={() => { setGeneratedUrl(''); setReason(''); setSharedWithName(''); }}
                  variant="outline"
            ...
            <div className="flex flex-col items-center py-12 animate-in zoom-in-95 duration-300">
              ...
              <h3 className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight mb-2">Solicitud Enviada</h3>
              <p className="text-[11px] text-slate-500 dark:text-slate-400 text-center mb-8 max-w-[280px]">
                Tu solicitud ha sido enviada al Director DIP para aprobación. Recibirás una notificación cuando sea revisada.
              </p>
              <Button onClick={handleClose} className="h-11 px-8 bg-blue-600 hover:bg-blue-700 text-white text-[10px] font-bold tracking-widest rounded-2xl shadow-lg shadow-blue-500/20">
                CERRAR
              </Button>
            </div>
          ) : (
            <div className="space-y-5 animate-in fade-in duration-300">
              <div className="flex items-end gap-4">
                <div className="flex-1 space-y-2">
                  <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 ml-1">Versión</label>
                  {versions.length <= 1 ? (
                    <div className="w-full h-11 px-4 border border-slate-300 dark:border-gray-800 rounded-2xl bg-white dark:bg-white/[0.02] text-sm font-medium flex items-center text-slate-700 dark:text-slate-300 shadow-sm">
                      {versions[0]?.version_label || versions[0]?.version_number || 'Última versión'}
                    </div>
                  ) : (
                    <div className="relative">
                      ...
                      >
                        {versions.sort((a: any, b: any) => b.id - a.id).map((v: any) => (
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="relative">
                    <select
                      ...
                      className="w-full h-11 px-4 border border-slate-300 dark:border-gray-800 rounded-2xl bg-white dark:bg-white/[0.02] text-slate-800 dark:text-white text-sm font-medium appearance-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all pr-10 cursor-pointer outline-none shadow-sm"
                    >
                      <option value="24" className="bg-white dark:bg-[#1a1c1e]">24 horas</option>
                      <option value="48" className="bg-white dark:bg-[#1a1c1e]">48 horas</option>
                      <option value="72" className="bg-white dark:bg-[#1a1c1e]">72 horas</option>
                      <option value="168" className="bg-white dark:bg-[#1a1c1e]">1 semana</option>
                      <option value="custom" className="bg-white dark:bg-[#1a1c1e]">Personalizado</option>
                    </select>
                    <ChevronDown className="absolute right-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 pointer-events-none" />
                  </div>
                  {showCustomDuration && (
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 ml-1">
                  Destinatario <span className="text-[9px] font-normal normal-case tracking-normal text-slate-400">(opcional)</span>
                </label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 ml-1">Motivo *</label>
                <textarea
                  value={reason}
              ...
              <label className="flex items-center gap-3 p-3 bg-white dark:bg-white/[0.02] rounded-2xl border border-slate-300 dark:border-gray-800 cursor-pointer select-none shadow-sm">
                <input
                  type="checkbox"
                  checked={readOnly}
              ...
              >
                {loading ? 'PROCESANDO...' : isDipDirector ? 'GENERAR ACCESO' : 'SOLICITAR COMPARTICIÓN'}
              </Button>
            </div>
          )}
        </div>
