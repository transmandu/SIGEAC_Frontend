...
export const DownloadDocumentDialog = ({
  isOpen,
  onClose,
  doc,
  company,
}: DownloadProps) => {
  const [downloadMode, setDownloadMode] = useState<"document" | "version">(
    "document",
  );
  const [versionList, setVersionList] = useState<Version[]>([]);
  const [selectedVersionToDownload, setSelectedVersionToDownload] =
    useState<string>("");
  ...
  useEffect(() => {
    if (isOpen) {
      registerTour(
        "biblioteca-descarga",
        "Descargar Documento",
        bibliotecaDescargaSteps,
      );
    }
    return () => unregisterTour("biblioteca-descarga");
  ...
  const handleFinalDownload = async () => {
    ...
    try {
      ...
      const versionObj = versionList.find(
        (v) => String(v.id) === selectedVersionToDownload,
      );
      const label = isBase
        ? "VIGENTE"
      ...
      <DialogContent className="bg-white dark:bg-[#1a1c1e] border-none text-slate-900 dark:text-white sm:max-w-[480px] rounded-2xl overflow-hidden p-0 outline-none shadow-2xl">
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="space-y-3">
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`mt-1 w-4 h-4 rounded-full border-2 flex items-center justify-center ${downloadMode === "document" ? "border-blue-500" : "border-slate-300 dark:border-slate-600"}`}
                >
                  {downloadMode === "document" && (
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-tight">
                    Descarga el archivo más reciente registrado en el sistema.
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium mb-3 leading-tight">
                    Selecciona una versión anterior de la trazabilidad.
                  </p>

                  {downloadMode === "version" && (
                    <div
                      className="relative animate-in fade-in zoom-in-95 duration-200"
                      data-tour="biblioteca-download-version-select"
                    >
                      <select
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-100 dark:border-slate-800">
            <button
              data-tour="biblioteca-download-cancel"
              type="button"
              onClick={onClose}
            ...
            <button
              data-tour="biblioteca-download-submit"
              onClick={handleFinalDownload}
              disabled={
                isProcessing ||
                (downloadMode === "version" &&
                  (!selectedVersionToDownload ||
                    availableVersions.length === 0))
              }
