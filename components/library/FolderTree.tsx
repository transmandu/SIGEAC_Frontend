  ...
  return (
    <div>
      <div
        ...
        data-tour="biblioteca-folder-node"
      >
        {hasChildren ? (
          <button
            onClick={(e) => {
        ...
        {canManage && !isRoot && (
          <div className="" data-tour="biblioteca-folder-actions">
            <DropdownMenu>
              ...
              >
                <div className="" data-tour="biblioteca-folder-rename">
                  <DropdownMenuItem
                    onClick={() => onRename(node, departmentId, departmentName)}
                    className="gap-2 cursor-pointer text-xs"
                  >
                    <Pencil className="h-3.5 w-3.5 text-blue-500" /> Renombrar
                  </DropdownMenuItem>
                </div>
                <div className="" data-tour="biblioteca-folder-delete">
                  <DropdownMenuItem
                    onClick={() => onDelete(node, departmentId, departmentName)}
...
function FolderTree({
  ...
  useEffect(() => {
    if (!isMultiDept && departmentFolders.length > 0) {
      ...
      const same =
        ids.length === expandedDepts.length &&
        ids.every((v, i) => v === expandedDepts[i]);
      if (!same) {
      ...
      departmentFolders.forEach((d) => {
        if (
          d.folders.length === 0 &&
          !toggledDepts.current.has(d.departmentId)
        ) {
  ...
  const renderDept = (dept: DepartmentFolderGroup, idx: number, depth = 0) => {
    ...
    const isSelected =
      selectedDeptName === dept.departmentName && selectedFolderPath === "/";

    return (
      <div key={dept.departmentId}>
        <div
          className={`flex items-center gap-1 w-full p-1 rounded-lg transition-all ${isSelected ? "bg-blue-50 dark:bg-blue-900/20" : "hover:bg-slate-100 dark:hover:bg-slate-800/50"} ${!isSelected && isDeptExpanded ? "bg-slate-50 dark:bg-slate-800/30" : ""}`}
          data-tour="biblioteca-dept-row"
        >
          {isMultiDept && (
            <button
              onClick={(e) => {
                e.stopPropagation();
                toggleDept(dept.departmentId, dept.departmentName);
              }}
              className="p-1.5 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md shrink-0 text-slate-500"
            >
              {isDeptExpanded || (dept.folders.length > 0 && !isLoading) ? (
            ...
            {isLoading && dept.folders.length === 0 ? (
              ...
              <FolderNodeRow
                ...
                onToggleFolder={(path: string) =>
                  handleToggleFolder(dept.departmentId, path)
                }
                expandedFolders={expandedFolders}
              />
            )}

            {Array.isArray(dept.descendants) &&
              dept.descendants.map((childDept, ci) => (
