...
import { X, Loader2, Send, CheckCircle2, XCircle, Clock, FileText, MessageSquare, Eye, Copy, Check, Share2 } from 'lucide-react';
...
export default function ShareRequestsPanel({ company, onClose }: ShareRequestsPanelProps) {
  const { user } = useAuth();
  ...
  const isDipDirector = useMemo(() => {
    ...
    const isDirector = user.employee?.some((emp: any) => {
      ...
      const deptAcronym = emp.department?.acronym?.toUpperCase() || '';
      ...
      <div className="flex-1 overflow-y-auto">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center px-6">
            ...
            <p className="text-sm font-bold text-slate-400 dark:text-slate-500">No hay solicitudes</p>
            <p className="text-[11px] text-slate-400 mt-1">Los directores pueden solicitar compartir documentos desde el menú de acciones.</p>
          </div>
        ) : (
          <div className="divide-y divide-slate-100 dark:divide-slate-800/50">
            {requests.map((req: any) => (
              <div key={req.id} className="p-5 hover:bg-slate-50 dark:hover:bg-white/[0.02] transition-colors">
                ...
                {/* Acciones para DIP Director / Superuser */}
