...
export function ReturnWarehouseDialog({ serial, id }: ReturnWarhouseProps) {
  ...
  const { mutateAsync, isPending, isError } =
    useReturnToWarehouse(selectedCompany?.slug);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>
            Retorno de Articulo:{" "}
            <span className="italic font-medium">{serial}</span>
          </DialogTitle>
          <DialogDescription>
        ...
        <h1 className="text-center text-2xl font-bold">
          ¿Seguro que desea retornar el articulo al almacén?
        </h1>
        ...
        <Button onClick={() => setOpen(false)} className="bg-rose-500">
          Cancelar
        </Button>
      </DialogContent>
    </Dialog>
