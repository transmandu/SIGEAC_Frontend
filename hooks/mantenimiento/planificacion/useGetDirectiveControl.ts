...
const fetchDirectiveControl = async (company: string | undefined, id: string | number | undefined): Promise<DirectiveControl> => {
  const { data } = await axios.get(`/${company}/directive-controls/${id}`);
  return data;
};

export const useGetDirectiveControl = (company: string | undefined, id: string | number | undefined) => {
