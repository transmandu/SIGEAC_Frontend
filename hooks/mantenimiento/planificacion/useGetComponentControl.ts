...
const fetchComponentControl = async (company: string | undefined, id: string | number | undefined): Promise<ComponentControl> => {
  const { data } = await axios.get(`/${company}/component-controls/${id}`);
  return data;
};

export const useGetComponentControl = (company: string | undefined, id: string | number | undefined) => {
