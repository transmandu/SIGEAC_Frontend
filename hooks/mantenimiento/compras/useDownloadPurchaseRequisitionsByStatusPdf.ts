 ...
 * blob ese JSON llega como Blob y hay que leerlo para mostrar el mensaje real.
 */
...
const fetchPurchaseRequisitionsByStatusPdf = async (
  ...
  try {
    const { data } = await axios.get(
      `/${company}/${locationId}/requisition-orders/purchases-by-status/pdf`,
      {
        params: { statuses, ...(type ? { type } : {}) },
...
export const useDownloadPurchaseRequisitionsByStatusPdf = () => {
  return useMutation<
    Blob,
    Error,
    {
      ...
      locationId: string;
      statuses: string[];
      /** Omitido, el reporte sale con requisiciones de ambos tipos. */
      type?: string;
    }
  >({
    mutationFn: ({ company, locationId, statuses, type }) =>
      fetchPurchaseRequisitionsByStatusPdf(company, locationId, statuses, type),
  });
};
