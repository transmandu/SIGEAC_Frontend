...
export const useCreateFlightControl = () => {
  ...
  const createMutation = useMutation({
    mutationFn: async ({
      ...
      data: CreateFlightControlData | CreateFlightControlData[];
      company: string;
    }) => {
...
export const useUpdateFlightControl = () => {
  ...
  const updateMutation = useMutation({
    mutationFn: async ({
      ...
      id: string;
      data: Partial<CreateFlightControlData> & EditReasonValue;
      company: string;
    }) => {
    ...
    onError: (error) => {
      toast.error("Oops!", {
        description:
          editReasonErrorFrom(error) ?? "No se pudo actualizar el vuelo...",
      });
      console.log(error);
    },
  });
