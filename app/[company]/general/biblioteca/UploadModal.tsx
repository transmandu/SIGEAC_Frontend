...
export default function UploadModal({
  ...
  useEffect(() => {
    if (isOpen) {
      registerTour(
        "biblioteca-upload",
        "Subir Documento",
        bibliotecaUploadSteps,
      );
    }
    return () => unregisterTour("biblioteca-upload");
      ...
      const fetchData = async () => {
        ...
        try {
          ...
          if (!isSuperUser && userDeptId) {
            availableDepts = availableDepts.filter(
              (d: any) => Number(d.id) === Number(userDeptId),
            );
          }
  ...
  const flattenFolders = (
    nodes: FolderNode[],
    prefix = "",
  ): { id: string; name: string; path: string; label: string }[] => {
      ...
      <div className="bg-white dark:bg-[#1a1c1e] rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden border border-slate-300 dark:border-gray-800 animate-in zoom-in-95 duration-200">
        ...
        <div className="bg-white dark:bg-gray-800/60 px-6 py-4 border-b border-slate-200 dark:border-gray-700 flex justify-between items-center">
          <div
            ...
            data-tour="biblioteca-upload-title"
          >
            <div className="p-1.5 bg-blue-100 dark:bg-blue-900/30 rounded-lg">
              <UploadCloud className="h-5 w-5 text-blue-600 dark:text-blue-400" />
            </div>
            <div>
              <h2 className="text-lg font-bold text-slate-800 dark:text-white tracking-tight uppercase">
                Subir documento
              </h2>
              {!isSuperUser && (
          ...
          <button
            ...
            className="text-slate-400 hover:text-red-500 transition-colors p-1 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-full"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <form
          ...
          className="p-6 space-y-5 bg-slate-50 dark:bg-[#1a1c1e]"
        >
          {/* Nombre Doc */}
          <div className="space-y-1.5" data-tour="biblioteca-upload-name">
            <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 flex items-center gap-2">
              <FileText className="h-3.5 w-3.5 text-blue-500" /> Nombre del
              Documento
            </label>
            <input
              type="text"
          ...
          {/* Versión Label */}
          <div className="space-y-1.5" data-tour="biblioteca-upload-version">
            <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 flex items-center gap-2">
          ...
          <div className="grid grid-cols-2 gap-4">
            {/* Depto Select */}
            <div className="space-y-1.5" data-tour="biblioteca-upload-depto">
              <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 flex items-center gap-2">
            ...
            <div
              ...
              data-tour="biblioteca-upload-categoria"
            >
              <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 flex items-center gap-2">
              ...
              <div className="relative">
                ...
                >
                  ...
                  <option
                    value="otro"
                    className="text-blue-500 font-bold dark:bg-gray-800"
                  >
                    + AGREGAR NUEVA...
                  </option>
                </select>
          ...
          {hasFolders && (
            <div className="space-y-1.5">
              ...
              <div className="relative" data-tour="biblioteca-upload-carpeta">
                <select
          ...
          {/* Vigencia */}
          <div className="space-y-2" data-tour="biblioteca-upload-vigencia">
            <label className="text-[11px] font-bold uppercase tracking-widest text-slate-500 dark:text-gray-400 flex items-center gap-2">
          ...
          {hasExpiry && (
            <div
              className="space-y-1.5 animate-in slide-in-from-top-2 duration-300"
              data-tour="biblioteca-upload-expiry"
            >
          ...
          {/* Drag and Drop */}
          <div className="pt-2" data-tour="biblioteca-upload-dropzone">
            <label
            ...
            >
              <div className="flex flex-col items-center justify-center text-center px-4 pointer-events-none">
                {file && !isDragging ? (
                  ...
                  <UploadCloud
                    className={`h-7 w-7 mb-1 transition-colors ${isDragging ? "text-blue-500" : "text-slate-400"}`}
                  />
                )}
          ...
          <div className="flex gap-3 pt-4 border-t border-slate-200 dark:border-gray-700">
            <button
              data-tour="biblioteca-upload-cancel"
              type="button"
              onClick={handleInternalClose}
            ...
            <button
              data-tour="biblioteca-upload-save"
              type="submit"
