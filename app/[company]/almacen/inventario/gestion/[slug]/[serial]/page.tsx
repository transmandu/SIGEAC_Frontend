...
const ArticlePage = () => {
  ...
  const { mutate, data, isPending, isError } = useGetArticle(
    selectedStation!,
    params.slug,
    params.serial,
  ...
  if (isPending) {
    return (
      <ContentLayout title="Cargado item...">
  ...
  return (
    <ContentLayout title={`Vista de Articulo`}>
      ...
      <p className="text-muted-foreground text-sm text-center mb-6 italic">
        Aquí puede ver los detalles de un articulo/componente en especifíco
      </p>
      <div className="flex flex-col gap-4 max-w-6xl mx-auto">
        <div className="max-w-7xl flex flex-col lg:flex-row gap-2 justify-center items-center w-full pb-6">
          ...
          <div className="flex flex-col gap-2">
            <Label>Serial</Label>
            <Input
              defaultValue={data?.article.serial}
          ...
          <div className="flex flex-col gap-2">
            ...
            <Input
              disabled
              defaultValue={data?.article.condition}
              className="w-[180px] disabled:opacity-100"
              placeholder="EJ: 234ABAC"
            />
          </div>
              ...
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={cn(
                    ...
                    !data?.shell_time.fabrication_date &&
                      "text-muted-foreground"
                  )}
                >
                  <CalendarIcon className="mr-2 h-4 w-4" />
                  {data?.shell_time.fabrication_date ? (
                    format(data?.shell_time.fabrication_date, "PPP", {
                      locale: es,
                    })
                  ) : (
          ...
          <div className="grid grid-cols-2 gap-6 h-[230px]">
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                ...
                defaultValue={data?.article.brand}
              />
            </div>
            {/* zone */}
            <div className="flex flex-col gap-2">
              ...
              <Input
                disabled
                className="disabled:opacity-100"
                defaultValue={data?.article.zone}
              />
            </div>
            {/* CALENDAR */}
            <div className="flex flex-col gap-2">
              ...
              <Popover>
                <PopoverTrigger asChild>
                  ...
                  >
                    <CalendarIcon className="mr-2 h-4 w-4" />
                    {data?.hard_time.calendary_date ? (
                      format(data?.hard_time.calendary_date, "PPP", {
                        locale: es,
                      })
                    ) : (
                      <span>Pick a date</span>
            ...
            <div className="flex flex-col gap-2">
              ...
              <Input
                disabled
                className="disabled:opacity-100"
                defaultValue={data?.hard_time.hour_date}
              />
            </div>
            {/* cycles date */}
            <div className="flex flex-col gap-2">
              ...
              <Input
                disabled
                className="disabled:opacity-100"
                defaultValue={data?.hard_time.cycle_date}
              />
            </div>
            {/* batch */}
            <div className="flex flex-col gap-2">
              ...
              <Input
                disabled
                className="disabled:opacity-100"
                defaultValue={data?.article.part_number}
              />
            </div>
          </div>
            ...
            <div className="flex flex-row-reverse gap-4 items-center  justify-center">
              <div className="flex flex-col lg:flex-row gap-2">
                <CertificatesDialog
                  ...
                  certificates={data?.article.certificates}
                />
              </div>
              {/* isFather */}
        ...
        {data?.childrens && (
          <>
            <h1 className="font-bold text-2xl italic text-center my-2">
              Subcomponentes - {data.article.serial}
            </h1>
            <div className="grid grid-cols-1 md:grid-cols-4 max-w-6xl mt-4 gap-4">
              {data?.childrens.map((c, index) => (
                <ChildrenInfoCard
                  key={c.child_component_id}
                  c={c}
                  index={index}
                />
              ))}
