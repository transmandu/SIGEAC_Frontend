...
const WarehouseInventoryPage = () => {
  ...
  const { data: articles, isLoading, isError } = useGetAllWarehouseArticles( selectedCompany?.slug, selectedStation!);
  ...
  if (isLoading) {
    return (
      <ContentLayout title='Inventario de Almacén'>
        ...
        <div className='flex items-center justify-between'>
          <div>
            ...
            <p className='text-sm text-muted-foreground mt-2'>
              Gestiona las cantidades de todos los artículos en el almacén
            </p>
          </div>
          <Button 
            onClick={handleSave} 
            disabled={!hasChanges || updateQuantities.isPending}
            className='flex items-center gap-2'
          >
            {updateQuantities.isPending ? (
            ...
            <CardContent>
              <div className='space-y-4'>
                {categoryArticles.map((article) => (
                  <div 
                    key={article.id} 
                    className='grid grid-cols-1 md:grid-cols-3 gap-4 p-4 border rounded-lg bg-card'
                  >
                    {/* Categoría/Artículo */}
