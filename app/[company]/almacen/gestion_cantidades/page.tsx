...
import { useUpdateConsumableArticleQuantities } from "@/actions/mantenimiento/almacen/articulos/useUpdateConsumableArticleQuantities";
const GestionCantidadesPage = () => {
  ...
  const { updateQuantities } = useUpdateConsumableArticleQuantities();
  ...
  const getModifiedArticles = useCallback(() => {
    return Object.entries(quantities).filter(([articleId, quantity]) => {
      ...
      batches?.forEach((batch) => {
        const article = batch.articles.find(
