...
export interface IArticleSimple {
  ...
  batch_name: string;
  batch_id: number;
  /** Clave del grupo cuando el servidor agrupa. */
  group_key?: string;
...
/**
 * Filas del servidor a la forma de la tabla. El servidor ya las manda en el
 * orden final —y, si agrupa, con los grupos contiguos y completos—, así que
 * aquí no se reordena nada.
 */
export const flattenArticles = (
  rows: WarehouseInventoryArticle[],
): IArticleSimple[] =>
  rows.map((article) => ({
    id: article.id,
    ...
    lot_number: article.lot_number ?? undefined,
    zone: article.zone ?? "",

    // No normalizar 0 -> 1: un consumible agotado es 0.
    quantity: Number(article.quantity ?? 0),
    stock: article.stock ?? undefined,

    status: article.status,
    status_since: article.status_since,
    condition: article.condition ?? "N/A",
    image: article.image ?? undefined,
    article_type: article.category,
    batch_name: article.batch_name ?? "",
    is_hazardous: article.is_hazardous,
    batch_id: article.batch_id,
    group_key: article.group_key ?? undefined,

    has_documentation: article.has_documentation,
    unit: (article.unit as Unit | null) ?? undefined,
    aircraft: (article.aircraft as Aircraft | null) ?? undefined,
    tool: article.tool ?? undefined,

    component:
      (article.category === "COMPONENT" || article.category === "PART") &&
      article.expiration_date
        ? { expiration_date: article.expiration_date, fabrication_date: null }
        : undefined,

    dimension: article.dimension ?? undefined,

    consumable:
      article.category === "CONSUMABLE"
        ? {
            expiration_date: article.expiration_date,
            shelf_life: article.shelf_life,
            unit: (article.unit as Unit | null) ?? undefined,
          }
        : undefined,
  }));
...
/**
 * Lo que las columnas necesitan de la página: el filtro de estado, que viaja
 * al servidor por su propio parámetro, y las condiciones de la compañía para
 * las opciones del filtro de condición.
 */
export interface WarehouseColumnOptions {
  statusFilter?: string;
...
const buildBaseCols = (
...
): AppColumnDef<IArticleSimple>[] => [
  {
    ...
    filterFn: (row, _id, value) => {
      ...
      return row.original.__isGroup
        ? matches(row.original) || (row.original.subRows ?? []).some(matches)
    ...
    cell: ({ row }) =>
      ...
      row.original.__isGroup ? null : (
        <ArticleImageCell
          ...
          alt={row.original.batch_name || row.original.part_number}
        />
      ),
    enableSorting: false,
    enableHiding: false,
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex-col text-center max-w-xs mx-auto line-clamp-2 leading-tight">
          {c ? (
            <>
...

  {
    ...
    filterFn: (row, _id, value) => {
      ...
      const matches = (article: IArticleSimple) =>
        toolStatus
          ? String(article.tool?.status ?? "")
              .trim()
              .toUpperCase() === toolStatus.toUpperCase()
...
// COMPONENTE / PARTE: la cantidad es el stock del número de parte.
export const buildComponenteCols = (
  options: WarehouseColumnOptions,
): AppColumnDef<IArticleSimple>[] => [
  ...buildBaseCols(options),
  quantityCol,
  actionsCol,
];
...
export const getColumnsByCategory = (
  ...
  if (cat === "TOOL") return buildHerramientaCols(options);
  if (cat === "CONSUMABLE") return buildConsumibleCols(options);
  if (cat === "COMPONENT" || cat === "PART")
    return buildComponenteCols(options);
  return buildAllCategoriesCols(options);
};

/**
 * Agrupa artículos CONSECUTIVOS con la misma clave, respetando el orden en que
 * llegó la lista. El servidor manda cada grupo contiguo y completo.
 */
