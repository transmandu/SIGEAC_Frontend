...
function globalComponentControlFilter(row: { original: ComponentControl }, term: string) {
...
export function DataTable<TData extends RowData>({ columns, data }: DataTableProps<TData>) {
  ...
  const table = useTable({
    ...
    globalFilterFn: globalComponentControlFilter as any,
    ...
    <div>
      <div className="flex items-center justify-between gap-3 py-4">
        <ActionTriggerButton asChild>
          <Link href={`/${selectedCompany?.slug}/planificacion/control_componentes/crear`}>
            <PlusCircle className="mr-2 h-4 w-4" />
            Crear Control de Componentes
          </Link>
        </ActionTriggerButton>
              ...
              <TableRow className="hover:bg-transparent">
                <TableCell colSpan={columns.length} className="h-40">
                  <div className="flex flex-col items-center justify-center gap-2 text-center">
                    ...
                    <p className="text-sm font-medium text-muted-foreground">No se encontró ningún control de componentes</p>
