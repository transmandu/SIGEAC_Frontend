...
import { AlertTriangle, Clock, Info, Radio, Search, SquarePen, Wrench } from "lucide-react";
...
const AvionicsControlDetailPage = () => {
  ...
  const { data: control, isLoading, isError } = useGetAvionicsControl(selectedCompany?.slug, id);
  ...
  const filtered = useMemo(() => {
    ...
    return activeItems.filter((item) => {
      ...
      if (status !== "all" && status !== "ON_CONDITION" && item.status_computed !== status) return false;
      if (needle) {
    ...
    return (
      <ContentLayout title="Control de Aviónica">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>No se pudo cargar el control de aviónica.</AlertDescription>
        </Alert>
      </ContentLayout>
    );
  }
      ...
      <div className="flex flex-col gap-6">
        ...
        <div className="flex flex-col gap-2 border-b pb-4 sm:flex-row sm:items-end sm:justify-between">
          ...
          <ActionTriggerButton asChild>
            <Link href={`/${company}/planificacion/control_avionica/editar/${control.id}`}>
        ...
        <FormSection icon={Info} title="Información General">
          <div className="grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3 md:grid-cols-6">
            ...
            <InfoItem label="Por condición" value={activeItems.length - scheduledCount} />
          </div>
        </FormSection>
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          <div className="relative w-full sm:w-72">
            ...
            <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar por descripción, P/N o S/N..." className="h-10 pl-9 text-sm" />
          </div>
          <Select value={category} onValueChange={setCategory}>
            ...
            <SelectContent>
              ...
              {Object.entries(AVIONICS_CATEGORY_LABELS).map(([value, label]) => (
          ...
          <Button type="button" variant={onlyHazardous ? "default" : "outline"} size="sm" onClick={() => setOnlyHazardous((v) => !v)} className="h-10 gap-1.5">
            <AlertTriangle className="size-4" />
            Mercancía peligrosa
          </Button>
        </div>

        <FormSection icon={Radio} title="Equipos de Aviónica" hint="Una fila por tarea; los equipos por condición no tienen plazo, solo se listan y se verifican.">
          {!filtered.length ? (
            <p className="text-sm italic text-muted-foreground">Ningún equipo coincide con el filtro.</p>
          ) : (
            <div className="overflow-x-auto rounded-lg border border-slate-400/40 dark:border-slate-600/40">
              <Table className="table-fixed">
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="bg-muted/40 font-semibold">Equipo</TableHead>
                    <TableHead className={cn(COL.task, "bg-muted/40 font-semibold")}>Tarea</TableHead>
                  ...
                  {filtered.map((item) => (
                    <Fragment key={item.id}>
                      {item.tasks.map((task, t) => {
                        const computed = task.computed ? computeMaintenanceItem(task) : null;
                          ...
                          <TableRow key={task.id ?? t} className={cn(meta?.row, "transition-colors hover:bg-primary/[0.03]")}>
                            {t === 0 && (
                              <TableCell rowSpan={item.tasks.length} className="align-top font-medium">
                                ...
                                <span className="block truncate text-xs text-muted-foreground">P/N {item.part_number} · S/N {item.serial}</span>
                                ...
                                <span className="mt-1 flex flex-wrap items-center gap-1">
                                  <Badge variant="outline" className="text-[10px]">{AVIONICS_CATEGORY_LABELS[item.category]}</Badge>
                                  {item.is_hazardous && (
                                    <Tooltip>
                                      <TooltipTrigger asChild>
                            ...
                            <TableCell className={cn(COL.task, "truncate")}>
                              <span className="block truncate">{AVIONICS_ACTION_LABELS[task.action]}</span>
                              {task.is_on_condition && <span className="block truncate text-xs italic text-muted-foreground">Por condición</span>}
                            </TableCell>
                            {computed ? (
                              <>
                              ...
                              <TableCell colSpan={7} className="text-sm text-muted-foreground">
                                Sin plazo — se verifica en tierra en cada inspección y se certifica en el 43-005.
                              </TableCell>
                            )}
                          </TableRow>
                        );
