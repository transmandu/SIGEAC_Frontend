...
export const TaskDetailsDialog = ({
  ...
  const [currentStep, setCurrentStep] = useState<'assign' | 'schedule'>('assign')
  ...
  const onSubmit = async (values: z.infer<typeof assignmentFormSchema>) => {
    //Vas a separar los values en dos objetos: uno para la tarea y otro para la programación. La tarea se puede llamar updateTaskData y la programación updateScheduleData.
    console.log(values)
      ...
      <DialogContent className="max-w-3xl max-h-full">
        <DialogHeader>
          ...
          <DialogDescription className="text-center">
            {currentStep === 'assign' ? 'Asignación de responsables' : 'Programación de la tarea'}
          </DialogDescription>
        </DialogHeader>
                ...
                <div className="space-y-2 flex space-x-4">
                  <div>
                    ...
                    <Calendar
                      ...
                      onSelect={(date) => date && setValue("scheduling.startDate", date)}
                  ...
                  <div className='space-y-3 w-full'>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setValue("scheduling.totalHours", parseInt(e.target.value) || 0)
                        }
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setValue("scheduling.hoursPerDay", parseInt(e.target.value) || 0)
                        }
                      />
                    </div>
                  ...
                  <div className="space-y-2">
                      {scheduling?.events?.length ? (
                        <div className="space-y-2">
                          <FormLabel className='p-2'>Eventos Programados</FormLabel>
                          <div className="border rounded-md divide-y max-h-[200px] overflow-y-auto">
                            {scheduling.events.map((event, index) => (
                              <div key={index} className="p-3">
                                <div className="font-medium">{event.title}</div>
                                <div className="text-sm text-muted-foreground">
                                  {format(event.start, 'PPP', {locale: es})} - {format(event.start, 'p')} a {format(event.end, 'p')}
                                </div>
                              </div>
                            ))}
                          </div>
            ...
            <DialogFooter className="sm:justify-between mt-5">
              {currentStep === 'assign' ? (
                <>
                  ...
                  {selectedTask.status === "ABIERTO" && selectedTask.technician_responsable && !isEditing && (
                    <div className='flex gap-2 items-center'>
                      ...
                      >
                        {updateTaskStatus.isPending && <Loader2 className="animate-spin mr-2 h-4 w-4" />}
                        Confirmar Cierre
                      </Button>
                    </div>
                  )}
                ...
                <>
                  <Button variant="outline" onClick={() => setCurrentStep('assign')} className="flex items-center gap-1">
                    <ChevronLeft className="h-4 w-4" />
                    Volver a asignación
                  </Button>
                  <Button onClick={() => onOpenChange(false)}>Guardar programación</Button>
                </>
              )}
            </DialogFooter>
          </form>
