...
import { AircraftAssingment, MaintenanceAircraft } from "@/types"
...
const fmtNumber = (n: any) => {
    ...
    const num = Number(n)
    return isNaN(num) ? "N/A" : num.toLocaleString('es-ES', { minimumFractionDigits: 0, maximumFractionDigits: 3 })
}
...
export function PlanificationAircraftTab({ aircraft }: { aircraft: MaintenanceAircraft & { aircraft_assignments?: any[], id: number } }) {
    ...
    const assignments = useMemo(() => {
        return (aircraft.aircraft_assignments || []).filter(a => a.removed_date === null) as AircraftAssingment[]
    }, [aircraft.aircraft_assignments])

    // Lista plana para la pestaña de búsqueda (incluye hijos recursivamente)
    const filteredFlat = useMemo(() => {
        const flat: AircraftAssingment[] = [];
