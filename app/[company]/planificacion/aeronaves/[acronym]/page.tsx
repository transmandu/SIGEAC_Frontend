...
const PartRow = ({
  ...
  return (
    <AccordionItem
      value={`${p.part_number}-${depth}-${index}`}
          ...
          <div className="flex-1">
            ...
            <div className="text-xs text-muted-foreground mt-1 flex gap-3 flex-wrap">
              <span className="flex items-center gap-1">
                <Gauge className="h-3 w-3" /> TSN:{" "}
                {fmtNumber(p.time_since_new ?? p.part_hours)}
              </span>
              <span className="flex items-center gap-1">
                <Layers className="h-3 w-3" /> CSN:{" "}
                {fmtNumber(p.cycles_since_new ?? p.part_cycles)}
              </span>
            </div>
          </div>
        </div>
...
export default function AircraftDetailsPage() {
  ...
  const { data: aircraft, isLoading } = useGetMaintenanceAircraftByAcronym(
    acronym,
          ...
          <div className="max-w-7xl mx-auto space-y-4">
            ...
            <div className="flex items-center gap-2 text-xs">
              <Badge variant="outline" className="gap-1">
                <Plane className="h-3.5 w-3.5" /> Aeronave
              </Badge>
                  ...
                  <div className="space-y-1 min-w-0">
                    ...
                    <CardDescription className="flex flex-wrap items-center gap-3">
                      <span className="flex items-center gap-1">
                        ...
                        {labelFor(aircraft.manufacturer)}
                      </span>
                      <Separator orientation="vertical" className="h-4" />
                      <span className="flex items-center gap-1">
                        <MapPin className="h-4 w-4" />{" "}
                        {labelFor(aircraft.location)}
                      </span>
                      <Separator orientation="vertical" className="h-4" />
                      <span className="flex items-center gap-1">
                        <FileText className="h-4 w-4" /> Cliente:{" "}
                        {labelFor(aircraft.client)}
                      </span>
                    </CardDescription>
                  </div>
                </div>
              ...
              <CardContent>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                  <Stat
                    icon={Gauge}
                    label="Flight Hours"
                    value={fmtNumber(aircraft.flight_hours)}
                  />
                  <Stat
                    icon={Layers}
                    label="Flight Cycles"
                    value={fmtNumber(aircraft.flight_cycles)}
                  />
                  <Stat
                    icon={Calendar}
                    label="Fabricación"
                  ...
                  <Stat
                    icon={Puzzle}
                    label="Partes Instaladas"
                    value={aircraft.aircraft_parts?.length ?? 0}
                  />
                </div>
              </CardContent>
            </Card>
                ...
                <Card>
                  <CardHeader className="pb-2">
                    <CardTitle className="text-sm flex items-center gap-2">
                      <Puzzle className="h-4 w-4" /> Partes instaladas
                    </CardTitle>
                    <CardDescription className="text-xs">
                      Estructura jerárquica de partes y subpartes
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
              ...
              <div className="flex flex-col-reverse gap-4">
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2">
                    <Input
                      placeholder="Buscar por PN o nombre de parte…"
                      className="h-9"
                    />
                    <p className="text-xs text-muted-foreground">
                      (Wire up a un estado/filtro si deseas hacerlo
                      interactivo).
                    </p>
                  </CardContent>
                </Card>
