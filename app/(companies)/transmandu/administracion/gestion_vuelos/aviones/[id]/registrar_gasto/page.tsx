...
const cash_movement_detailsSchema = z.object({
  ...
  details: z.string().min(2, { message: "El detalle debe tener al menos 2 caracteres." }),
...
const movementSchema = z.object({
  ...
  details: z.string({ message: "La descripción es requerida" }).min(2, { message: "La descripción debe tener al menos 2 caracteres." }),
  ...
  cash_movement_details: z.array(cash_movement_detailsSchema).min(1, { message: "Debe agregar al menos un gasto." }),
...
const formSchema = z.object({
  ...
  movements: z.array(movementSchema).min(1, { message: "Debe agregar al menos un movimiento." }),
})
...
export default function AircraftExpensesPage() {
  ...
  const { data: allCategories, isLoading: isAllCategoriesLoading } = useGetCategory()
  ...
  async function onSubmit(formData: z.infer<typeof formSchema>) {
    ...
    await createCashMovementForAircraft.mutateAsync({ id, formData: transformedData })
  }
  ...
  return (
    <ContentLayout title="Registro de Gastos">
      <div className="space-y-6">
        <h1 className="text-5xl font-bold text-center">Registro de Gastos de Aeronave</h1>
        <p className="text-muted-foreground text-center italic">
          Ingrese la información para registrar los gastos asociados a la aeronave.
        </p>
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col space-y-3">
                ...
                {movementFields.map((movement, movementIndex) => (
                  <div key={movement.id} className="border p-4 rounded-lg space-y-4 mb-4 bg-gray-50">
                    <div className="flex justify-between items-center">
                      <h4 className="font-semibold text-3xl text-gray-800">Movimiento {movementIndex + 1}</h4>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <Select disabled={isCashesLoading} onValueChange={field.onChange} value={field.value}>
                                <SelectTrigger>
                        ...
                        <FormField
                          ...
                          render={({ field }) => (
                            <FormItem>
                              ...
                              <Select disabled={isVendorLoading} onValueChange={field.onChange} value={field.value}>
                                <SelectTrigger>
                    ...
                    <div className="mt-6">
                        ...
                        <Button
                          ...
                          onClick={() => {
                            ...
                            form.setValue(`movements.${movementIndex}.cash_movement_details`, [
                              ...currentExpenses,
                              { accountant_id: "", category_id: "", details: "", amount: "" },
                            ])
                          }}
                        >
                          <PlusCircle className="mr-2 h-4 w-4" />
                      ...
                      <ScrollArea className="h-[300px]">
                        {form.watch(`movements.${movementIndex}.cash_movement_details`)?.map((_, expenseIndex) => {
                          const currentAccountantId = form.watch(
                            `movements.${movementIndex}.cash_movement_details.${expenseIndex}.accountant_id`
                          )

                          return (
                            <div key={expenseIndex} className="border p-4 rounded-lg mb-4 bg-white">
                                  ...
                                  <FormField
                                    ...
                                    render={({ field }) => (
                                      <FormItem>
                                        ...
                                        >
                                          <SelectTrigger>
                                            <SelectValue placeholder={isAccountLoading ? <Loader2 className="animate-spin" /> : "Seleccione una cuenta..."} />
                                          </SelectTrigger>
                                          <SelectContent>
                                      ...
                                      return (
                                        <FormItem>
                                          ...
                                          <Select
                                            onValueChange={field.onChange}
                                            value={field.value}
                                            disabled={!currentAccountantId || isAllCategoriesLoading}
                                          >
                                            <SelectTrigger>
                                              <SelectValue
                                                placeholder={
                                            ...
                                            <SelectContent>
                                              {
                                                isAllCategoriesLoading && <Loader2 className="size-4 animate-spin" />
                                              }
                                              ...
                                              {
                                                filteredCategories.length === 0 && !isAllCategoriesLoading && (
            ...
            <Button type="submit" disabled={createCashMovementForAircraft.isPending} className="self-end">
              {createCashMovementForAircraft.isPending ? (
